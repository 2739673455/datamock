...
class Layer3Generator(BaseGenerator):
    ...
    def fetch_platform_coupon_start(self) -> datetime:
        row = db.fetch_one(
            "SELECT MIN(created_at) AS min_created_at FROM org_institution"
        )
                ...
                rows.append(
                    {
                        ...
                        "question_type_id": question_types[seed["question_type_code"]][
                            "id"
                        ],
                        "stem": seed["stem"],
    ...
    def generate_homework_questions(self) -> int:
        ...
        for homework in homeworks:
            candidates = questions[
                (homework["institution_id"], homework["category_id"])
            ]
    ...
    def build_coupon_row(
        ...
        created_floor = (
            institution["created_at"] if institution else self.platform_coupon_start
        )
        ...
        return {
            ...
            "threshold_amount": amount * Decimal("5")
            if coupon_type == "cash"
            else Decimal("0"),
    ...
    def generate_coupon_series_rels(self) -> int:
        ...
        all_series = [
            series for rows in series_by_institution.values() for series in rows
        ]
        rows: list[dict[str, Any]] = []
        for coupon in coupons:
            candidates = (
            ...
            created_at = self.random_datetime(
                max(coupon["created_at"], series["created_at"])
            )
            rows.append(
                {
                    "coupon_id": coupon["id"],
    ...
    def generate_exposure_logs(self) -> int:
        ...
        def rows():
            for index in range(count):
                ...
                yield {
                    ...
                    + timedelta(seconds=self.random.randint(0, 60)),
                    "created_at": created_at,
                }
        ...
        return self.stream_rows(
            ...
            rows(),
            total_rows=count,
            build_step_name="build series_exposure_log",
        )
    ...
    def generate_visit_logs(self) -> int:
        ...
        def rows():
            for exposure in exposures[::2]:
                enter_at = exposure["exposed_at"] + timedelta(
                    seconds=self.random.randint(5, 300)
                )
                ...
                yield {
                    ...
                    "visit_source": self.visit_source_from_scene(
                        exposure["exposure_scene"]
                    ),
        ...
        return self.stream_rows(
            "series_visit_log",
            rows(),
            total_rows=len(exposures[::2]),
            build_step_name="build series_visit_log",
        )
    ...
    def generate_search_logs(self) -> int:
        ...
        return self.stream_rows(
            ...
            rows(),
            total_rows=count,
            build_step_name="build series_search_log",
        )
    ...
    def generate_favorites(self) -> int:
        ...
        series_rows = self.fetch_series()
        count = min(
            int(GENERATION_DEFAULTS["favorites"]), len(users) * len(series_rows)
        )
        ...
        return self.stream_rows(
            ...
            rows(),
            total_rows=count,
            build_step_name="build series_favorite",
        )
    ...
    def generate_cart_items(self) -> int:
        ...
        return self.stream_rows(
            ...
            rows(),
            total_rows=count,
            build_step_name="build shopping_cart_item",
        )
    ...
    def generate_consultations(self) -> int:
        ...
        def rows():
            for index in range(count):
                ...
                yield {
                    ...
                    + timedelta(minutes=self.random.randint(0, 60)),
                    "created_at": created_at,
                    "updated_at": self.random_datetime(created_at),
                }
        ...
        return self.stream_rows(
            ...
            rows(),
            total_rows=count,
            build_step_name="build consultation_record",
        )
    ...
    def generate_coupon_receives(self) -> int:
        ...
        return self.stream_rows(
            ...
            build_step_name="build coupon_receive_record",
        )
    ...
    def pick_questions(
        ...
        return [
            candidates[(offset + index) % len(candidates)] for index in range(count)
        ]
    ...
    def fetch_by_code(
        self, table_name: str, code_column: str
    ...
    def fetch_questions_by_institution_and_category(
        ...
        for row in rows:
            result.setdefault((row["institution_id"], row["category_id"]), []).append(
                row
            )
        return result
