...
class Layer6Generator(BaseGenerator):
    ...
    def generate_teacher_compensation(self) -> dict[str, int]:
        ...
        for session in sessions:
            ...
            grouped[
                (session["institution_id"], session["teacher_id"], settle_period)
            ].append(session)
        ...
        bill_count = self.insert_rows(
            "teacher_compensation_bill", self.strip_ids(bill_rows)
        )
        item_count = self.insert_rows(
            "teacher_compensation_item", self.strip_ids(item_rows)
        )
    ...
    def generate_channel_commission(self) -> dict[str, int]:
        ...
        approvers = self.fetch_staff_users_by_institution(
            ("sales", "management", "academic")
        )
        grouped: dict[tuple[int, int, str], list[dict[str, Any]]] = defaultdict(list)
        for row in rows:
            settle_anchor = row["paid_at"] or row["created_at"]
            settle_period = settle_anchor.strftime("%Y-%m")
            grouped[(row["institution_id"], row["channel_id"], settle_period)].append(
                row
            )
        ...
        bill_count = self.insert_rows(
            "channel_commission_bill", self.strip_ids(bill_rows)
        )
        item_count = self.insert_rows(
            "channel_commission_item", self.strip_ids(item_rows)
        )
        return {"bill_count": bill_count, "item_count": item_count}

    def generate_ugc_moderation_tasks(self) -> int:
        target = int(GENERATION_DEFAULTS["ugc_moderation_target"])
        moderators = self.fetch_staff_users_by_institution(
        ...
        def rows():
            for index, candidate in enumerate(candidates[:target], start=1):
                ...
                if status != "pending" and moderator_pool:
                    moderator_user_id = moderator_pool[index % len(moderator_pool)][
                        "user_id"
                    ]
        ...
        return self.stream_rows(
            "ugc_moderation_task",
            rows(),
            total_rows=min(target, len(candidates)),
            build_step_name="build ugc_moderation_task",
        )

    def generate_risk_alerts(self) -> dict[str, int]:
        target = int(GENERATION_DEFAULTS["risk_alert_target"])
        handlers = self.fetch_staff_users_by_institution(
            ("service", "academic", "management")
        )
        ...
        for alert_index, alert in enumerate(alert_rows, start=1):
            ...
            record_count = (
                1
                if alert["alert_status"] == "in_progress"
                else self.random.randint(1, 3)
            )
            last_handled_at = persisted["detected_at"]
            for index in range(record_count):
                handler = institution_handlers[
                    (alert_index + index) % len(institution_handlers)
                ]
                ...
                if alert["alert_status"] == "closed" and index == record_count - 1:
                    final_result = self.random.choice(
                        ("resolved", "false_positive", "confirmed_risk")
                    )
                    ...
                    action_type = self.random.choice(
                        ("review", "contact_user", "freeze_account")
                    )
                    action_result = self.random.choice(
                        ("pending_follow_up", "confirmed_risk")
                    )
