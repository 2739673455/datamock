...
def validate_layer3() -> list[str]:
    ...
    if invalid_homework_questions:
        raise ValueError(
            "session_homework_question_rel has invalid score or institution"
        )
    ...
    if invalid_exam_score_sum:
        raise ValueError(
            "session_exam_question_rel score sum does not match exam total"
        )
...
def validate_layer5(now: datetime | None = None) -> list[str]:
    ...
    if missing_attendance:
        raise ValueError(
            "session_attendance does not cover all effective cohort learners"
        )
    ...
    if invalid_video_events:
        raise ValueError(
            "session_video_play_event has invalid timing or event position"
        )
    ...
    if invalid_homework_submission:
        raise ValueError(
            "session_homework_submission has invalid status, relation, or time"
        )
    ...
    if missing_homework_submission:
        raise ValueError(
            "session_homework_submission does not cover all effective learners"
        )
    checks.append(
        "homework submissions cover all effective learners and terminal states"
    )
    ...
    if invalid_exam_submission:
        raise ValueError(
            "session_exam_submission has invalid status, relation, or time"
        )
    ...
    if missing_exam_submission:
        raise ValueError(
            "session_exam_submission does not cover all effective learners"
        )
    ...
    if invalid_posts:
        raise ValueError(
            "cohort_discussion_post has invalid relation or parent linkage"
        )
    ...
    if invalid_tickets:
        raise ValueError(
            "service_ticket has invalid relation, state, or refund linkage"
        )
    ...
    if invalid_survey:
        raise ValueError(
            "service_ticket_satisfaction_survey has invalid closure or score relation"
        )
...
def validate_layer6() -> list[str]:
    ...
    if invalid_teacher_bill:
        raise ValueError(
            "teacher_compensation_bill has invalid relation, status, or amount"
        )
    ...
    if invalid_teacher_bill_agg:
        raise ValueError(
            "teacher_compensation_bill aggregate values do not match item totals"
        )
    ...
    if invalid_teacher_item:
        raise ValueError(
            "teacher_compensation_item has invalid teacher, session, or amount rules"
        )
    ...
    if invalid_commission_bill:
        raise ValueError(
            "channel_commission_bill has invalid relation, status, or amount"
        )
    ...
    if invalid_commission_bill_agg:
        raise ValueError(
            "channel_commission_bill aggregate values do not match item totals"
        )
    ...
    if invalid_commission_item:
        raise ValueError(
            "channel_commission_item has invalid order linkage or commission formula"
        )
    ...
    if invalid_risk_alert:
        raise ValueError(
            "risk_alert_event has invalid object linkage or alert-type fields"
        )
    ...
    if invalid_moderation:
        raise ValueError(
            "ugc_moderation_task has invalid content linkage or status fields"
        )
...
def validate_layer7() -> list[str]:
    ...
    if empty_tables:
        raise ValueError(
            f"Layer7 contains empty core table(s): {', '.join(empty_tables)}"
        )
    ...
    if duplicate_keys:
        raise ValueError(
            f"Layer7 unique business keys duplicated: {', '.join(duplicate_keys)}"
        )
    ...
    checks.append(
        "learning, review, and service records trace back to real fulfillment or orders"
    )
