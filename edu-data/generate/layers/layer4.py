...
class Layer4Generator(BaseGenerator):
    ...
    def build_order_contexts(self) -> list[dict[str, Any]]:
        ...
        while len(contexts) < order_count and candidate_index < len(candidates) * 3:
            ...
            coupon = self.pick_coupon_for_order(
                candidate, coupons_by_user, order_status
            )
            context = self.build_order_context(
                candidate, coupon, order_status, len(contexts) + 1
            )
    ...
    def build_payment_record(
        ...
        paid_at = (
            order["paid_at"]
            if payment_status in {"paid", "partial_refunded", "refunded"}
            else None
        )
        refund_amount = (
            order["refund_amount"]
            if payment_status in {"partial_refunded", "refunded"}
            else None
        )
        refund_at = None
        if refund_amount is not None:
        ...
        updated_at = max(
            value
            for value in (order["created_at"], paid_at, refund_at)
            if value is not None
        )
        return {
            "institution_id": candidate["institution_id"],
    ...
    def build_refund_requests(
        ...
        for context in extra_candidates[:extra_limit]:
            ...
            apply_amount = self.money(
                context["order_item"]["payable_amount"] * Decimal("0.50")
            )
    ...
    def pick_coupon_for_order(
        ...
        for coupon in coupons_by_user[candidate["user_id"]]:
            if coupon["id"] in self.linked_coupon_receive_ids:
                continue
            if (
                coupon["institution_id"] is not None
                and coupon["institution_id"] != candidate["institution_id"]
            ):
                continue
    ...
    def calculate_discount(
        ...
        if coupon["coupon_type"] == "discount" and coupon["discount_rate"] is not None:
            return self.money(
                total_amount * (Decimal("1.00") - coupon["discount_rate"])
            )
        return Decimal("0.00")
    ...
    def fetch_order_candidates(self) -> list[dict[str, Any]]:
        ...
        rows = cart_rows + consultation_rows + favorite_rows + visit_rows + search_rows
