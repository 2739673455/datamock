...
class Layer5Generator(BaseGenerator):
    ...
    def run(self) -> None:
        ...
        counts["service_ticket_follow_record"] = (
            self.generate_service_ticket_follow_records()
        )
            ...
            for rel in fulfillments.get(session["cohort_id"], []):
                ...
                if status in {"present", "late"}:
                    base_time = datetime.combine(
                        session["teaching_date"], session_start_time
                    )
                    offset = (
                        self.random.randint(0, 25)
                        if status == "late"
                        else self.random.randint(-5, 8)
                    )
                    checkin_time = max(
                    ...
                    remark = "学员已提前请假。"
                created_end = datetime.combine(
                    session["teaching_date"], session_end_time
                )
    ...
    def generate_video_play(self) -> int:
            ...
            for video in videos:
                for rel in fulfillments.get(video["cohort_id"], []):
                    ...
                    progress_percent = (
                        Decimal("100.00")
                        if completed
                        else Decimal(str(self.random.randint(8, 96)))
                    )
                    ...
                    watched_seconds = max(
                        30,
                        last_position_seconds
                        - self.random.randint(0, max(1, last_position_seconds // 5)),
                    )
                    ...
                    ended_at = self.cap_at_now(
                        ...
                        + timedelta(
                            seconds=min(
                                duration, watched_seconds + self.random.randint(0, 180)
                            )
                        )
                    )
                    ...
                    yield {
                        ...
                        "exit_reason": "play_complete"
                        if completed
                        else self.random.choice(EXIT_REASONS),
        ...
        return self.stream_rows(
            ...
            rows(),
            total_rows=None,
            build_step_name="build session_video_play",
        )
    ...
    def generate_video_play_events(self) -> int:
        ...
        def rows():
            for play in play_rows:
                ...
                if play["completed_flag"] == 1:
                    ...
                    resume_pos = max(
                        pause_pos, int(play["last_position_seconds"] * 0.36)
                    )
                    ...
                    events.extend(
                        [
                            (
                                ...
                                pause_pos,
                                min(
                                    play["started_at"] + timedelta(minutes=6),
                                    complete_time,
                                ),
                            ),
                            (
                                ...
                                resume_pos,
                                min(
                                    play["started_at"] + timedelta(minutes=8),
                                    complete_time,
                                ),
                            ),
                    ...
                    events.extend(
                        [
                            (
                                ...
                                seek_pos,
                                min(
                                    play["started_at"] + timedelta(minutes=5), end_time
                                ),
                            ),
                            ("exit", play["last_position_seconds"], end_time),
                        ]
                    )
                for index, (event_type, position_seconds, event_time) in enumerate(
                    events, start=1
                ):
                    yield {
                        ...
                        "position_seconds": min(
                            position_seconds, play["duration_seconds"]
                        ),
        ...
        return self.stream_rows(
            ...
            rows(),
            total_rows=None,
            build_step_name="build session_video_play_event",
        )
    ...
    def generate_exam_submissions(self) -> int:
        ...
        for exam in exams:
            for rel in fulfillments.get(exam["cohort_id"], []):
                ...
                latest_start = exam["deadline_at"] - timedelta(
                ...
                if attempt_status in {"in_progress", "submitted", "timeout"}:
                    start_at = self.random_datetime(
                        max(created_at, exam["window_start_at"]), latest_start
                    )
                if attempt_status == "submitted":
                    ...
                    score_value = Decimal(
                        str(self.random.randint(55, int(exam["total_score"])))
                    )
                ...
                rows.append(
                    {
                        ...
                        "is_pinned": 1
                        if index == 0 and self.random.random() < 0.25
                        else 0,
                        "is_closed": 0,
                        "view_count": 0,
                        "reply_count": 0,
    ...
    def generate_discussion_posts(self) -> int:
        ...
        for topic in topics:
            ...
            for index in range(direct_count):
                author = cohort_participants[
                    (topic["id"] + index) % len(cohort_participants)
                ]
                created_at = self.cap_at_now(
                    topic["created_at"] + timedelta(hours=index + 1)
                )
                direct_rows.append(
                    {
                        ...
                        "content_text": POST_CONTENTS[
                            (topic["id"] + index) % len(POST_CONTENTS)
                        ],
    ...
    def generate_cohort_reviews(self) -> int:
        ...
        for rel in self.fetch_reviewable_fulfillments():
            ...
            if isinstance(cohort_start_at, date) and not isinstance(
    ...
    def generate_service_tickets(self) -> int:
        ...
        for item in self.fetch_service_ticket_candidates():
            ...
            ticket_type = (
                "refund"
                if refund is not None and self.random.random() < 0.7
                else self.random.choice(("after_sales", "complaint"))
            )
            assignee_pool = assignees[item["institution_id"]]
            assignee = (
                assignee_pool[sequence % len(assignee_pool)] if assignee_pool else None
            )
            created_at = (
            ...
            updated_at = max(
                value
                for value in (created_at, first_response_at, closed_at)
                if value is not None
            )
            rows.append(
                {
                    ...
                    if ticket_type == "refund" and refund is not None
                    else None,
                    "ticket_type": ticket_type,
                    "ticket_source": "system_auto"
                    if ticket_type == "refund"
                    else self.random.choice(SERVICE_TICKET_SOURCES),
                    "priority_level": "high"
                    if ticket_type == "complaint"
                    else self.random.choice(PRIORITY_LEVELS),
    ...
    def generate_service_ticket_follow_records(self) -> int:
        ...
        for ticket in tickets:
            ...
            if not institution_staff:
                continue
            follow_count = (
                1 if ticket["ticket_status"] == "pending" else self.random.randint(2, 4)
            )
            for index in range(follow_count):
                follower = institution_staff[
                    (ticket["id"] + index) % len(institution_staff)
                ]
                created_at = self.cap_at_now(
                    ticket["created_at"] + timedelta(hours=index + 1)
                )
                followed_at = created_at
                if (
                    ticket["closed_at"] is not None
                    and followed_at > ticket["closed_at"]
                ):
                    followed_at = ticket["closed_at"]
                    created_at = followed_at
                follow_type = (
                    "refund_review"
                    if ticket["ticket_type"] == "refund" and index == 0
                    else self.random.choice(FOLLOW_TYPES)
                )
    ...
    def fetch_discussion_participants_by_cohort(
        self,
