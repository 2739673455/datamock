...
class Layer2Generator(BaseGenerator):
    ...
    def created_at_ceiling(self, *candidates: datetime | date) -> datetime:
        normalized = [self.now]
        normalized.extend(
            self.normalize_datetime(candidate) for candidate in candidates
        )
    ...
    def generate_series(self) -> int:
        ...
        creators = self.fetch_staff_users_by_institution(
            categories=("academic", "management")
        )
        ...
        for index, (institution, template, delivery_mode, category) in enumerate(
            work_items
        ):
            ...
            rows.append(
                {
                    ...
                    "description": (
                        f"{template['product_name']}下的{template['series_name']}课程。"
                    ),
                    "cover_url": (
                        f"https://cdn.example.com/course/{template['series_code']}.jpg"
                    ),
    ...
    def generate_series_cohorts(self) -> int:
        ...
        head_teachers = self.fetch_staff_by_institution_and_campus(
            categories=("academic",)
        )
        institution_head_teachers = self.fetch_staff_by_institution(
            categories=("academic",)
        )
        series_rows = self.fetch_series()
        ...
        for series in series_rows:
            ...
            for index in range(1, cohorts_per_series + 1):
                ...
                open_date = self.cohort_open_date(
                    series["id"], index, cohorts_per_series
                )
                min_open_date = max(
                    series["created_at"],
                ...
                rows.append(
                    {
                        ...
                        "cohort_name": (
                            f"{series['series_name']} {open_date.strftime('%Y%m')}期"
                        ),
    ...
    def generate_series_cohort_courses(self) -> int:
        ...
        for cohort in cohorts:
                ...
                course_templates = [
                    {
                        ...
                        "total_hours": int(GENERATION_DEFAULTS["sessions_per_course"])
                        * 2,
                        "module_keywords": "[]",
                    }
                    for index in range(1, fallback_count + 1)
                ]
            cohort_end_date = cohort["end_date"] or cohort["start_date"] + timedelta(
                days=55
            )
            ...
            for index, template in enumerate(course_templates, start=1):
                start_date = cohort["start_date"] + timedelta(
                    days=(index - 1) * module_days
                )
                end_date = (
                    cohort_end_date
    ...
    def generate_series_cohort_sessions(self) -> int:
        ...
        for course in courses:
            ...
            for index, teaching_date in enumerate(session_dates, start=1):
                ...
                created_at = self.random_datetime(
                    course["created_at"],
                    self.created_at_ceiling(
                        datetime.combine(teaching_date, time(hour=8))
                    ),
                )
                rows.append(
                    {
    ...
    def generate_session_assets(self) -> int:
        sessions = self.fetch_sessions_with_context()
        uploaders = self.fetch_staff_users_by_institution(
            categories=("teacher", "academic")
        )
        rows: list[dict[str, Any]] = []
        for session in sessions:
    ...
    def generate_session_homeworks(self) -> int:
        ...
        for session in sessions:
            ...
            due_at = datetime.combine(
                session["teaching_date"], time(hour=23, minute=59)
            ) + timedelta(days=7)
    ...
    def cohort_open_date(self, series_id: int, index: int, total: int) -> date:
        ...
        window_index = min(
            window_count - 1, ((index - 1) * window_count) // max(total, 1)
        )
    ...
    def pick_room(
        ...
        if campus_id is not None:
            campus_candidates = [
                room for room in candidates if room["campus_id"] == campus_id
            ]
