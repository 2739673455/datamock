...
class Layer1Generator(BaseGenerator):
    ...
    def generate_users(self) -> int:
        ...
        def rows():
            for index in range(1, total + 1):
                ...
                birthday = created_at.date() - timedelta(
                    days=self.random.randint(18 * 365, 45 * 365)
                )
                real_name = self.name()
                last_login_at = (
        ...
        return self.stream_rows(
            ...
            rows(),
            total_rows=total,
            build_step_name="build sys_user",
        )
    ...
    def generate_staff_profiles(self) -> int:
        ...
        for campus in campuses:
            ...
            institution_roles = [
                role
                for role in roles
                if role["institution_id"] == campus["institution_id"]
            ]
            for local_no in range(int(GENERATION_DEFAULTS["staff_per_campus"])):
                ...
                preferred_category = self.category_for_department(
                    str(dept["dept_name"])
                )
                role = next(
                    (
    ...
    def generate_campus_managers(self) -> int:
        ...
        for campus in self.fetch_campuses():
            ...
            created_at = self.random_datetime(
                max(
                    self.org_window_start, campus["created_at"], selected["created_at"]
                ),
                self.org_window_end,
            )
            rows.append(
    ...
    def generate_student_profiles(self) -> int:
        ...
        for index, user in enumerate(users, start=1):
            identity_code = self.random.choices(
                identity_codes,
                weights=[
                    45 if code == "in_school_student" else 20 for code in identity_codes
                ],
                k=1,
            )[0]
            is_student = identity_code == "in_school_student"
            goal_code = self.random.choice(
                k12_goal_codes if is_student else adult_goal_codes
            )
            created_at = self.random_datetime(
            ...
            rows.append(
                {
                    ...
                    "grade_id": self.random.choice(grades)["id"]
                    if is_student
                    else None,
                    "school_name": self.random.choice(SCHOOL_NAMES)
                    if is_student
                    else None,
                    ...
                    "industry_name": None
                    if is_student
                    else self.random.choice(INDUSTRIES),
                    "job_role_name": None
                    if is_student
                    else self.random.choice(JOB_ROLES),
                    "career_stage": None
                    if is_student
                    else self.random.choice(CAREER_STAGES),
    ...
    def fetch_staff_roles(self) -> list[dict[str, Any]]:
        return db.fetch_all(
            "SELECT * FROM org_staff_role ORDER BY institution_id, sort_no"
        )
