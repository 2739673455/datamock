...
class OrderQuoteRequest(BaseModel):
    cohortId: int = Field(description="班次 ID，对应 series_cohort.id；用于试算班次价格。")
    couponReceiveRecordId: int | None = Field(
        default=None, description="领券记录 ID，对应 coupon_receive_record.id；不使用优惠券时为空。"
    )


class OrderCreateRequest(BaseModel):
    studentId: int = Field(description="学员档案 ID，对应 student_profile.id，且必须属于当前用户。")
    cohortId: int = Field(description="班次 ID，对应 series_cohort.id；班次必须有效。")
    couponReceiveRecordId: int | None = Field(
        default=None, description="领券记录 ID，对应 coupon_receive_record.id；不使用优惠券时为空。"
    )
    orderSourceChannelId: int | None = Field(
        default=None, description="订单来源渠道 ID，对应 dim_channel.id；不传则使用系统默认渠道。"
    )
    remark: str | None = Field(default=None, description="下单备注。")


class OrderCancelRequest(BaseModel):
    reason: str = Field(description="取消原因，会写入订单取消记录。")
...
def ensure_channel(channel_id: int) -> None:
    if (
        fetch_one("SELECT id FROM dim_channel WHERE id = %s AND yn = 1", (channel_id,))
        is None
    ):
...
@router.post("/orders/quote", summary="试算订单金额")
def quote_order(
    ...
    discount = coupon_discount(
        body.couponReceiveRecordId, current_user_id, total, body.cohortId
    )
    return ok(
        {
            ...
            "availableCoupons": available_coupon_receives(
                current_user_id, total, body.cohortId
            ),
        }
    )
...
def create_order(
    ...
    validate_order_source_channel(
        current_user_id, body.cohortId, body.orderSourceChannelId
    )
...
def list_orders(
    ...
    status: Annotated[
        str | None,
        Query(
            description="订单状态。可选值：pending、paid、completed、cancelled、partial_refunded、refunded。"
        ),
    ] = None,
...
@router.get("/orders/{order_id}", summary="查询订单详情")
