...
class ConsultationCreateRequest(BaseModel):
    ...
    contactMobile: str = Field(description="咨询人联系电话，用于顾问回访。")
    consultContent: str | None = Field(default=None, description="咨询内容，可为空。")
    sourceChannelId: int = Field(description="来源渠道 ID，对应 dim_channel.id。")
...
@router.post("/cohorts/{cohort_id}/consultations", summary="提交班次咨询")
def create_consultation(
    body: Annotated[
        ConsultationCreateRequest, Body(description="创建咨询记录请求体。")
    ],
    cohort_id: Annotated[int, Path(description="班次 ID，对应 series_cohort.id。")],
...
@router.get("/me/consultations", summary="分页查询我的咨询")
