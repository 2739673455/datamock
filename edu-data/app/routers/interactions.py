...
class CohortReviewCreateRequest(BaseModel):
    ...
    anonymousFlag: int = Field(
        default=0, description="是否匿名。可选值：0 非匿名、1 匿名。"
    )
...
@router.post("/cohorts/{cohort_id}/reviews", summary="提交班次评价")
def create_cohort_review(
    body: Annotated[
        CohortReviewCreateRequest, Body(description="提交班次评价请求体。")
    ],
...
@router.get("/cohorts/{cohort_id}/reviews", summary="分页查询班次评价")
def list_cohort_reviews(
    ...
    only_mine: Annotated[
        bool, Query(alias="onlyMine", description="是否只查询当前用户自己的评价。")
    ] = False,
