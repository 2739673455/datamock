...
class SampleData:
    ...
    def inapplicable_coupon_order_context(self) -> dict[str, int]:
        ...
        for row in coupon_series:
            coupon_series_map.setdefault(int(row["coupon_id"]), set()).add(
                int(row["series_id"])
            )
        ...
        for receive in receives:
            ...
            for cohort in cohorts:
                ...
                category_ok = not applicable_categories or bool(
                    series_category_map.get(series_id, set()) & applicable_categories
                )
                if not (series_ok and category_ok):
                    return {
                        ...
                        "coupon_receive_record_id": int(
                            receive["coupon_receive_record_id"]
                        ),
                    }
