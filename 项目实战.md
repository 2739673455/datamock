...
### 2.7 财务单据智能风险审核系统
- 项目目标：实现一个对接元年云待审批单据的智能风险审核系统，自动核对单据结构化字段与附件内容，识别金额、开支、消费行为和供应商风险，并生成可视化风险提示面板
- 交付形式：提供可运行前端、后端、数据库初始化脚本、元年云模拟接口、示例单据、示例附件、接口说明和完整审核演示链路

#### 2.7.1 开发范围
- 基础能力：登录鉴权、角色权限、审核会话、操作日志、任务状态管理
- 单据能力：单据查询、结构化字段展示、明细展示、附件下载、附件预览
- 解析能力：PDF 和 PNG 附件解析、OCR 识别、发票与合同关键字段提取、原文定位
...
- `系统管理员`：维护用户、角色、元年云接口配置、模型配置和系统参数
...
- `单据详情页`：展示单据基本信息、费用明细、付款明细、附件列表和元年云原始字段
...
- 信息完整后，系统按 `单据类型 + 单据编号` 调用元年云接口查询单据结构化字段和明细
- 系统读取附件清单并下载当前用户有权访问的附件
- 系统解析 PDF、PNG、JPG 附件，提取发票、合同、行程单和付款依据中的关键字段
- Agent 汇总单据字段、明细、附件解析结果、审核规则、市场价参考数据和供应商资料，生成风险分析结果
- 系统保存风险项、判断依据、证据位置、风险等级、处理建议和分析时间
- 元年云查询失败、附件下载失败或附件解析失败时，系统展示失败环节并允许重试
...
- `单据模块`：处理单据字段、费用明细、付款明细和附件列表展示
- `附件模块`：处理附件预览、解析文本、提取字段、证据定位和解析失败重试
...
- `审核模块`：处理人工复核意见、风险项确认和审批结果记录

#### 2.7.8 后端模块要求
- `认证与权限模块`：负责登录、当前用户识别、角色权限和单据数据权限校验
- `会话模块`：负责审核会话、消息记录、槽位状态和多轮上下文管理
- `元年云接入模块`：负责按单据类型和单据编号查询单据、查询附件清单和下载附件
- `单据模块`：负责单据结构化字段、明细和原始响应保存
- `附件模块`：负责附件保存、格式校验、访问控制和解析任务创建
- `文档解析模块`：负责 PDF 文本提取、图片 OCR、文档分类、字段提取和证据定位
- `规则引擎模块`：负责金额容差、费用标准、市场价区间和异常阈值校验
- `Agent 分析模块`：负责聚合结构化数据、附件内容、规则结果、市场价和供应商信息并生成风险结论
...
#### 2.7.9 数据表要求
- `users`：`id`、`username`、`display_name`、`password_hash`、`role`、`status`、`created_at`、`updated_at`
...
- `financial_documents`：`id`、`document_type`、`document_no`、`applicant_name`、`applicant_department`、`budget_department`、`payee_name`、`payee_account`、`expense_category`、`total_amount`、`currency`、`apply_date`、`reason_text`、`raw_payload_json`、`created_at`
- `document_line_items`：`id`、`document_id`、`item_type`、`item_name`、`expense_date`、`expense_location`、`quantity`、`unit_price`、`amount`、`remark`
- `document_attachments`：`id`、`document_id`、`external_attachment_id`、`file_name`、`file_type`、`file_size`、`file_path`、`download_status`、`parse_status`、`created_at`
...
- `GET /api/v1/yuannian/documents/{document_type}/{document_no}`：查询元年云单据结构化字段和明细
- `GET /api/v1/yuannian/documents/{document_type}/{document_no}/attachments`：查询元年云单据附件清单
- `POST /api/v1/documents/{document_id}/attachments/sync`：下载并同步单据附件
...
#### 2.7.11 任务状态与实时消息
- 附件下载状态至少包含 `pending`、`downloading`、`succeeded`、`failed`
- 附件解析状态至少包含 `pending`、`parsing`、`succeeded`、`failed`、`manual_review`
- 分析任务状态至少包含 `queued`、`querying_document`、`downloading_attachments`、`parsing_attachments`、`analyzing`、`succeeded`、`failed`、`cancelled`
...
- `attachment_status`：字段至少包含 `attachment_id`、`download_status`、`parse_status`
...
#### 2.7.13 安全边界与验收标准
- 不保存元年云明文密码、明文 token 和明文 cookie
- 单据字段、附件内容、供应商资料和分析结果按用户权限访问
- 附件下载地址必须校验来源、文件类型、文件大小和访问权限
- AI 分析结果只作为审批辅助依据，最终审批结果由有权限的人员人工确认
- 所有金额对比、规则判断和风险结论都必须保留数据来源和审计记录
- 能通过多轮交互补齐单据类型和单据编号，并保持已确认的会话信息
- 能从元年云模拟接口查询五类单据的结构化字段、明细和附件
- 能下载并解析 PDF、PNG、JPG 附件，展示解析文本、提取字段和证据位置
...
- 能通过示例数据跑通 `多轮澄清 -> 单据查询 -> 附件下载 -> 附件解析 -> Agent 分析 -> 风险展示 -> 人工复核` 完整链路
